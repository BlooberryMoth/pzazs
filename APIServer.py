...
class Server(BaseHTTPRequestHandler):
    ...
    def handle_GET_request_response(self, url: list[str], parameters: dict) -> list[list[tuple[str, str]], int, dict]:
        ...
        if url[0] == "":
            ...
            return headers, 200, {"userCount": user_count, "serverCount": server_count}
        ...
        # Returns files from /http/res
            ...
            try:
                with open(f"./http/{url}", "rb") as data: return [], 200, data.read()
            except: return [("Content-Type", "application/json")], *error(404, f"File \"./{url}\" not found.")

        elif url[0] == "login":
            if len(url) == 1: return [("Location", login_redirect)], 307, bytes("", 'utf-8')
